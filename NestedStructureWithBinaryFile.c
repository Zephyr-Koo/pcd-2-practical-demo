...
void saveFriendList()
{
    ...
    if (fOutput != NULL)
    {
        ...
        {
            fwrite(&friendList[i], sizeof(friendList[i]), 1, fOutput); // save one record at a time
        }
...
void displayFriendList()
{
    ...
    if (fInput != NULL)
    {
        ...
        do
        {
            fread(&friend, sizeof(friend), 1, fInput); // read one record at a time

            if (!feof(fInput)) // have we reached end-of-file?
            {
                printf("Name   : %s \n", friend.name);
        ...
        } while (!feof(fInput)); // continue reading until reaching end-of-file
